...
namespace Task_Management.Commands
{
    public class HelpCommand : BaseCommand
    {
...

        {
            ...
            sb.AppendLine("22. Add comment to a task. - Input:([addcomment]  [taskId] [content] [nameAuthor])");
            sb.AppendLine("23. List all tasks. - Input:([listalltasks]  [all] or [ExactTitle])");
            sb.AppendLine("24.1. List bugs/stories/feedback only. - Input:([list]  [TypeOfTask] [Filter(status or assignee)] [title,priority,severity,size,rating])");
            sb.AppendLine("24.2. List bugs/stories/feedback only. - Input:([list]  [TypeOfTask] [Filter(status or assignee)] [and] [assignee] [title,priority,severity,size,rating])");
            sb.AppendLine("25.1. List task with assignee. - Input:([listtaskswithassignee] [Filter(status or assignee)]");
            sb.AppendLine("25.2. List task with assignee. - Input:([listtaskswithassignee] [Filter(status or assignee)] [and] [assignee]");
